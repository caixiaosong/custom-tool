export class XBitUtils {
    ...
    public static setBits(bitState: number, index: number, num: number): number {
        const bitOne: number = XBitUtils.createBitSetValue(num, index);
    ...
    public static clearBitsByNum(bitState: number, startIndex: number, clearNum: number): number {
        ...
        endIndex = Math.min(endIndex, XBitUtils.MAX_BIT_FOR_NUM);
        return XBitUtils.clearBitsByIndexs(bitState, startIndex, endIndex);
    }
     ...
     */
    public static clearBitsByIndexs(bitState: number, startIndex: number, endIndex: number = XBitUtils.MAX_BIT_FOR_NUM): number {
        ...
        const bitOneValue: number = XBitUtils.createBitSetValue(bitNum, startIndex);
     ...
     * @param endIndex 默认32
     */
    public static getBitCount(bitState: number, startIndex: number = 0, endIndex: number = XBitUtils.MAX_BIT_FOR_NUM): number {
        let bitNum: number = 0;
        for (let i: number = startIndex; i < endIndex; ++i) {
            if (XBitUtils.isBitSet(bitState, i)) {
                ++bitNum;
            }
        }
        return bitNum;
    ...
    public static unZipBit(bitState: number, index: number, zipBitNum: number): number {
        ...
        const bitOneValue: number = XBitUtils.createBitSetValue(zipBitNum, offset);
     ...
     * @param endIndex
     */
    public static arrToBit(arr: number[], startIndex: number = 0, endIndex: number = XBitUtils.MAX_BIT_FOR_NUM): number {
        let value: number = 0;
        // 防御编程
        if (arr == null) {// 防空
            return value;
        ...
        for (let i: number = startIndex; i < endIndex; ++i) {
            if (arr[i] !== 0) {
                value = XBitUtils.setBit(value, i);
            }
        }
        return value;
    }
    ...
    public static createBitSetValue(bitSetNum: number, offset: number = 0): number {
        ...
        for (let i: number = 0; i < bitSetNum; ++i) {
            value = XBitUtils.setBit(value, i + offset);
        }
        return value;
    }
}
